...
func TestUnpinCmdProperties(t *testing.T) {
	f := &factory.Factory{}
	cmd := newUnpinCmd(f)

	assert.Equal(t, "unpin <message-id|url>", cmd.Use)
	assert.Equal(t, "Unpin a message from the top of the message board", cmd.Short)
